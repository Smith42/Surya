...
def main():
    ...
    print("="*60)
    print("SURYA Filament Extraction - Inference Pipeline")
    print("="*60)
    
    # Download data (comment out if already downloaded)
    print("\n[1/8] Downloading model and configuration...")
    download_model_data()
    
    print("\n[2/8] Downloading SDO data...")
    sdo_data_root = download_sdo_data()
    
    # Load configuration and scalers
    print("\n[3/8] Loading configuration and scalers...")
    config = load_config()
    scalers = load_scalers()
    
    # Setup device
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"\n[4/8] Using device: {device}")
    if device == "cuda":
    ...
    print("\n[6/8] Loading pretrained weights...")
    model = load_model_weights(model, "data/Surya-1.0/surya.366m.v1.pt", device)
    
    # Create dataset and dataloader
    print("\n[7/8] Creating dataset...")
    # Use the inference data index that was downloaded
    ...
    dataset = create_dataset(config, scalers, sdo_data_root, index_path)
    print(index_path)
    print(dataset)
    ...
    # Run inference
    print(f"\n[8/8] Running inference on {len(dataset)} samples...")
